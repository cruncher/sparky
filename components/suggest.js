...
function listen(node, fn) {
	...
	function click(e) {
		var button = closest('[data-fn~="scope"]', e.target, e.currentTarget);
		var object = getScope(button);
		update(object);
	}
...
register('suggest', function(node, scopes) {
	if (tag(node) !== 'input') {
	...
	listen(tip.filter(isElementNode)[0], function(data) {
